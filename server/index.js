...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user._id, studentName: user.studentName },
      secretKey
    );
...
app.get("/api/get-topics/:subject/:unit", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const topics = selectedSubject.name
      .find((sub) => sub.subject_name === subject)
...
app.post("/api/add-subject", verifyToken, async (req, res) => {
  try {
    ...
    // Add the new subject to the database if it doesn't already exist
    const updatedData = await SubjectModel.findOneAndUpdate(
      { std, "name.subject_name": { $ne: subject } }, // Check if subject does not exist
      { $addToSet: { name: { subject_name: subject, units: [] } } }, // Add subject to array
      { new: true, upsert: true } // Ensure document is created if std does not exist
    );
...
app.post("/api/add-topic-description", verifyToken, async (req, res) => {
  try {
    ...
    const unitIndex = unitDocument.name.findIndex((u) =>
      u.units.some((u) => u.unit_name === unit)
    );
    ...
    const updatedData = await SubjectModel.findOneAndUpdate(
      ...
      {
        $set: {
          [`name.${unitIndex}.units.$[u].topics.$[t].topic_description`]:
            topic_description,
        },
      },
      {
        arrayFilters: [{ "u.unit_name": unit }, { "t.topic_name": topic }],
        new: true,
      }
    );
...
app.get("/api/get-units", async (req, res) => {
  try {
    ...
    const units = subject.name.find(
      (subj) => subj.subject_name === subjectName
    ).units;
...
app.get("/api/get-topics", async (req, res) => {
  try {
    ...
    const unit = subject.name
      .find((s) => s.subject_name === subjectName)
      ?.units.find((u) => u.unit_name === unitName);
    ...
    topics.forEach((topic) => {
      console.log(
        `Topic Name: ${topic.topic_name}, Topic Description: ${topic.topic_description}`
      );
    });
...
app.post("/searchTopic", async (req, res) => {
  try {
    ...
    const subjectName = result.name.find((subject) =>
      subject.units.some((unit) =>
        unit.topics.some((topic) => topic.topic_name === topicName)
      )
    ).subject_name;
    const unitName = result.name
      .find((subject) =>
        subject.units.some((unit) =>
          unit.topics.some((topic) => topic.topic_name === topicName)
        )
